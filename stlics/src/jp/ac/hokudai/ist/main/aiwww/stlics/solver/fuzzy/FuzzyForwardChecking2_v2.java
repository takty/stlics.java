 ...
 * @version 2012/07/31
 */
public class FuzzyForwardChecking2_v2 extends AbstractFuzzySolver implements SystematicSolver {

	private static enum BranchCode {CONTINUE, TERMINATE, UPDATED}
	 ...
	 * @param p ファジィ制約充足問題
	 */
	public FuzzyForwardChecking2_v2(Problem p) {
		...
		solutionWorstDeg_ = Math.max(0.0, p.worstSatisfactionDegree());
	}
	 ...
	 */
	public FuzzyForwardChecking2_v2(Problem p, double worstSatisfactionDegree) {
		this(p);
...
//			System.out.println("currentLevel " + currentLevel);
			...
			if(deg > solutionWorstDeg_) {  // 「大なり」のときは新たな解とする
				...
				solution_.set(pro_);
				bc = BranchCode.TERMINATE;  // 制限により探索終了
//				bc = BranchCode.UPDATED;
				if(targetDeg_ != UNSPECIFIED && targetDeg_ <= solutionWorstDeg_) {
					break;
				}  // 目標到達により探索終了
			}
		}
